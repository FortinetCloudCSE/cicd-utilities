...
- Populate `launch-runner-params.json` with the following values (keep secrets out of version control):
  ...
  * `RunnerVersion`: From the GitHub runner download section (for example `2.307.1`).
  * `GHAToken`: A registration token generated in the GitHub wizard. Tokens expire after 60 minutes—store them in AWS Systems Manager Parameter Store (SecureString) or fetch just-in-time when you deploy.
  * `HashCheck`: SHA-256 hash from the runner release asset page; ensures the download is trusted.
